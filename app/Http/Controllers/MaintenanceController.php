...
class MaintenanceController extends Controller
{
    ...
    public function maintenance_cost_view_page(){
        ...
        if($header_status==1){
            ...
            return view('admin.maintenance.cost_view',compact('menu_data'));
        }
        else{

            $system_data = $this->common->get_system_data();

            return view('admin.maintenance.cost_view_master',compact('menu_data','system_data'));
        }
    }

    public function maintenance_cost_grid (Request $request){
        
        DB::enableQueryLog();

        $menu_data = $this->common->get_page_menu_grid('maintenance.manage.add');
        ...
        if($search_value!=''){
            ...
            $total_data = DB::table('maintenance_costs as a')
                ->leftJoin('buildings as b', 'b.id', '=', 'a.building_id')
                ->leftJoin('levels as c', 'c.id', '=', 'a.level_id')
                ->leftJoin('units as d', 'd.id', '=', 'a.unit_id')
                ->leftJoin('users as e', 'e.id', '=', 'a.owner_id')
                ->leftJoin('reference_costs as f', 'f.id', '=', 'a.reference_cost_id')
                ->select('a.id')
                ->where('a.delete_status',0)
                ->get();

            $filter_data = DB::table('maintenance_costs as a')
                ->leftJoin('buildings as b', 'b.id', '=', 'a.building_id')
                ->leftJoin('levels as c', 'c.id', '=', 'a.level_id')
                ->leftJoin('units as d', 'd.id', '=', 'a.unit_id')
                ->leftJoin('users as e', 'e.id', '=', 'a.owner_id')
                ...
                ->where(function ($query) use ($search_value) {
                    $query->where('c.level_name','like',"%".$search_value."%")
                        ...
                        ->orWhere('f.cost_name','like',"%".$search_value."%");
                })
                ->get();

            $data = DB::table('maintenance_costs as a')
                ->leftJoin('buildings as b', 'b.id', '=', 'a.building_id')
                ->leftJoin('levels as c', 'c.id', '=', 'a.level_id')
                ->leftJoin('units as d', 'd.id', '=', 'a.unit_id')
                ->leftJoin('users as e', 'e.id', '=', 'a.owner_id')
                ->leftJoin('reference_costs as f', 'f.id', '=', 'a.reference_cost_id')
                ->select('a.id', 'a.cost', 'a.cost_date', 'a.cost_file', 'a.status', 'b.building_name', 'c.level_name', 'd.unit_name', 'e.name', 'f.cost_name')
                ->where('a.delete_status',0)
                ->where(function ($query) use ($search_value) {
                    $query->where('c.level_name','like',"%".$search_value."%")
                        ->orWhere('b.building_name','like',"%".$search_value."%")
        ...
        foreach($data as $value){
            ...
            $record_data[$sl]['id'] = $value->id;
            $record_data[$sl]['sl'] = $sl_start;
            $record_data[$sl]['cost'] = $value->cost;
            $record_data[$sl]['cost_date'] = $value->cost_date;
            $record_data[$sl]['cost_file'] = $value->cost_file;
